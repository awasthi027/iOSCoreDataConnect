...
public final class DataManager {
    ...
    private static var managedObjectModel: NSManagedObjectModel = {
        ...
        guard let modelURL = DataManager.dataModelBundle?.url(forResource: dataModelName, withExtension: "momd") else {
        ...
        FSLogInfo("Successfully create managed object model")
    ...
    private static var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        ...
        FSLogInfo("Successfully created persistance stored coordinator")
        return coordinator
    }()
...
extension DataManager {
    ...
    public static func fetchObjects<T: NSManagedObject>(entity: T.Type, predicate: NSPredicate? = nil, sortDescriptors: [NSSortDescriptor]? = nil, context: NSManagedObjectContext) -> [T] {
        ...
        do {
            ...
            FSLogError("Find error while fetching object entity \(entity):- \(error.localizedDescription)")
            return [T]()
        }
    }
    ...
    public static func fetchObject<T: NSManagedObject>(entity: T.Type, predicate: NSPredicate? = nil, sortDescriptors: [NSSortDescriptor]? = nil, context: NSManagedObjectContext) -> T? {
        ...
        do {
            ...
            FSLogError("Find error while fetching managed single object Entity \(entity):- \(error.localizedDescription)")
            return nil
        }
    }
}
    ...
    public static func deleteAllObjects() {
        ...
        for entityName in managedObjectModel.entitiesByName.keys {
            ...
            do {
                ...
                FSLogError("Find error while deleting managed object:- \(error.localizedDescription)")
            }
        }
    }
}
...
extension DataManager {
    ...
    public class func convertToJSONArray(moArray: [NSManagedObject]) -> Any {
        ...
        for item in moArray {
            ...
            for relation in item.entity.relationshipsByName {
                let key = relation.key
                if let object = item.value(forKey: key) as? NSManagedObject {
                    var subEntity: [String: Any] = [:]
                    for subAttributes in object.entity.attributesByName {
                        //check if value is present, then add key to dictionary so as to avoid the nil value crash
                        if let value = object.value(forKey: subAttributes.key) {
                            subEntity[subAttributes.key] = value
                        }
                    }
                    dict[key] = subEntity
                }
